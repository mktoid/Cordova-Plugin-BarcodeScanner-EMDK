...
class EMDKScannerInterface implements EMDKManager.EMDKListener, Scanner.StatusListener, Scanner.DataListener {
	...
	private boolean softReadPending = false;            ///<  Scan without the need of a hardware button is running
	...
	/**
	 * By default the scanner reads on hardware key press.
	 * This method starts ONE read without the need of a hardware key.
	 *
	 * @param callbackContext
	 */
	public void startSoftRead (CallbackContext callbackContext) {
		if (scanner != null && scannerEnabled && continuouslyReading) {
			try {
				softReadPending = true;
				scanner.cancelRead();
				scanner.triggerType = Scanner.TriggerType.SOFT_ALWAYS;
				scanner.read();
			} catch (ScannerException e) {
				EnterpriseBarcode.FailureCallback(callbackContext, "Exception whilst enabling soft read: " + e.getMessage());
			}
		} else {
			EnterpriseBarcode.FailureCallback(callbackContext, "Scanner is not enabled");
		}
	}

	/**
	 * Bring back the need of a hardware key press to start reading.
	 *
	 * @param callbackContext
	 */
	public void stopSoftRead (CallbackContext callbackContext) {
		if (softReadPending) {
			try {
				softReadPending = false;
				scanner.cancelRead();
				scanner.triggerType = Scanner.TriggerType.HARD;
				scanner.read();
			} catch (ScannerException e) {
				EnterpriseBarcode.FailureCallback(callbackContext, "Exception whilst stopping soft read: " + e.getMessage());
			}
		}
	}
